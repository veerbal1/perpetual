...
### Build Frustration Guard

During build tutoring, never combine a new protocol invariant with new Rust/Anchor syntax in the same teaching chunk.

Teach build concepts in this order:

1. Plain analogy
2. Concrete state table
3. One helper/input-output contract
4. Rust syntax for that helper
5. Composition with other helpers

If the user shows frustration, stop coding immediately. Validate the load, summarize the one current idea, and offer a reset or pause. Do not ask the user to keep writing code while emotionally overloaded.
...
- Before prompting code, run a Drift-alignment pass:
  - read the exact Drift source slice for this build unit
  - extract the invariant being preserved
  - name safe simplifications vs dangerous simplifications
  - update `drift-build/README.md` if the plan misses a bridge
...
If the user is emotionally overloaded, do not continue the hint ladder. Switch to reset mode: one analogy, one state table, no Rust syntax, no new helper composition.
...
- [ ]  In build mode, did I introduce only one new axis: concept, syntax, edge case, or composition?
- [ ]  If a Drift invariant appeared, did I separate it from Rust syntax and update the roadmap if needed?
