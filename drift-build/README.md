...
- **Current status (2026-05-10):** Build Slice 1 is partially shipped. On disk now: `Order::get_base_asset_amount_unfilled`, `Order::update_open_bids_and_asks`, `controller/orders.rs::update_order_after_fill`, `controller/position.rs::decrease_open_bids_and_asks` with update flag and zero clamps, `User::decrement_open_orders`, the focused fill receipt builder slice (`OrderAction`, `OrderActionExplanation`, `OrderActionRecord`, `get_order_action_record`), post-receipt counter cleanup helper `decrement_open_orders_after_full_fill`, Drift-shaped `state/fill_mode.rs` with `is_liquidation` / `is_ioc`, `Order.slot`, `Order::has_auction`, simplified `math::auction::calculate_auction_price`, simplified `Order::get_limit_price`, `FillMode::get_limit_price` with PlaceAndTake auction-percentage tests, `state::fulfillment::PerpFulfillmentMethod`, and `math::matching::do_orders_cross`. Study Slice 2 is closed: price crossing, maker direction, maker price as matched fill price, maker-vs-AMM ordering, `AMM(Some(price))`, `AMM(None)`, empty route list, Alice pipeline story, focused `OrderActionRecord` field tour, and cleanup/cancel taxonomy are locked. Latest assistant verification: full `cargo test` passed with 60 tests after the route-label and crossing-gate helpers. R4/R5 foundation debt is intentionally parked for this small order-progress/event/cleanup/fill-mode/route-helper slice only; return to it before any full trade demo or M1 close. Build only Drift-mirrored helper slices here. Do not add invented mini-only fill wrappers; if a dependency belongs to R8/R9/R10/R13/R23, park it rather than replacing it with a fake flow.
- **Study:**
...
- **Tests:** 🎯 `auction.rs` tests: copy Drift's auction vectors from `math/auction.rs::tests`, including boundary derivation from oracle/limit and current-price interpolation. Add `FillMode::PlaceAndTake` auction-percentage tests. Current mini route-helper tests cover `PerpFulfillmentMethod` shape and four `do_orders_cross` Long/Short crossing cases. Add price-band tests for oracle/TWAP divergence. Add cleanup tests: expired order and reduce-only-now-invalid order cancel with flat keeper reward only when filler is external. Add no-self-reward test for taker-as-filler. Add order-progress tests only through the R8 position-update boundary, so order filled state never moves without a position update.
- **Est.:** 3 days
...
| R7 | Study Slice 1 complete. Study Slice 2 closed: price crossing, maker direction, maker price as matched fill price, maker-vs-AMM ordering, `AMM(Some(price))`, `AMM(None)`, empty route list, Alice pipeline, focused `OrderActionRecord` field tour, and cleanup/cancel taxonomy. | Build Slice 1 in progress. Shipped Drift-mirrored helper slices/tests: unfilled amount with reduce-only safety, order progress after fill, open bid/ask decrease with update flag and clamps, `User::decrement_open_orders`, focused AMM-style `OrderActionRecord` receipt builder, post-receipt counter cleanup helper, `FillMode` basics, `Order.slot`, `Order::has_auction`, simplified auction price helper, simplified fixed-wall `Order::get_limit_price`, `FillMode::get_limit_price`, `PerpFulfillmentMethod`, and `do_orders_cross`. Still pending: fees, keeper rewards, emit inside real fill path, and real fill orchestration only after its Drift dependencies are studied/built |
...
- [ ] R7 · Dutch auction + fill flow helpers *(study Slice 1 complete; Study Slice 2 closed; Build Slice 1 partially shipped: order progress, open bid/ask decrease, open-order counter helpers, fill receipt builder, post-receipt counter cleanup, FillMode basics, simplified auction price helper, fixed-wall order price helper, `FillMode::get_limit_price`, `PerpFulfillmentMethod`, and `do_orders_cross`; no mini-only fake fill wrapper)*
