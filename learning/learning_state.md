...
## Current Position
- Ring: 7 of 26
- Active Concept: Ring 7 Build Slice 1 — standalone route helper bridge shipped 2026-05-10: `PerpFulfillmentMethod` enum + `math/matching.rs::do_orders_cross`. Next-step decision pending: continue exact helper slices only, or open Ring 8 when ready to unblock real position mutation.
- Status: IN PROGRESS - Ring 7 Build Slice 1 is partially shipped. Completed Drift-mirrored helper slices: `Order::get_base_asset_amount_unfilled`, `Order::update_open_bids_and_asks`, `controller/orders.rs::update_order_after_fill`, `controller/position.rs::decrease_open_bids_and_asks` with update flag and zero clamps, `User::decrement_open_orders`, the focused fill receipt builder slice (`OrderAction`, `OrderActionExplanation`, `OrderActionRecord`, `get_order_action_record`), post-receipt counter cleanup helper `decrement_open_orders_after_full_fill`, Drift-shaped `FillMode` with `is_liquidation` / `is_ioc`, the `FillMode::get_limit_price` helper path, `state::fulfillment::PerpFulfillmentMethod`, and `math::matching::do_orders_cross`. Latest assistant verification: full `cargo test` passed on 2026-05-10 with 60 tests after the route-label and crossing-gate helpers. R4/R5 foundation debt remains intentionally parked for this small order-progress/event/cleanup/fill-mode/route-helper slice only; return to it before any full trade demo or M1 close. Do not mark full Ring 7 complete yet; full AMM/maker route planner, fees, keeper rewards, emit inside real fill flow, and real fill orchestration are still pending.
...
- [ ] Build Slice 1 - IN PROGRESS: order-progress helpers, fill receipt shape, counter cleanup, FillMode basics, route-label/crossing helpers, and tests. Completed so far: unfilled amount including reduce-only safety, partial/full fill order progress, open bid/ask decrease with Drift-style update flag and clamps, `User::decrement_open_orders`, focused `OrderActionRecord` receipt builder for current AMM-style fill slice, `decrement_open_orders_after_full_fill` with partial/full/underflow tests, `FillMode` enum plus `is_liquidation` / `is_ioc`, `Order.slot`, `Order::has_auction`, `math::auction::calculate_auction_price`, simplified `Order::get_limit_price`, `FillMode::get_limit_price` with PlaceAndTake auction-percentage tests, `state::fulfillment::PerpFulfillmentMethod`, and `math::matching::do_orders_cross` with four direction-specific crossing tests. Still pending: emitting the receipt inside a real Drift-shaped fill path and the R8-owned position-update boundary; do not fake any of these with mini-only flow helpers.
...
- FulfillmentMethod (actual fill route: `AMM` or `Match`; studied conceptually, build still pending)
...
- Ring 7 next - continue Study Slice 2 in tiny chunks: bridge the locked Alice route-selection story back to Drift source in `math/fulfillment.rs::determine_perp_fulfillment_methods`, then study fees, keeper reward, cleanup/cancel paths, and exact `OrderActionRecord` fields. Do not build a fake fill wrapper; keep R8/R9/R13/R23 dependencies parked.
