...
## Current Position
- Ring: 7 of 26
- Active Concept: Ring 7 Study Slice 2 - real fill route after price-wall selection
- Status: IN PROGRESS - Ring 7 Build Slice 1 is partially shipped. Completed Drift-mirrored helper slices: `Order::get_base_asset_amount_unfilled`, `Order::update_open_bids_and_asks`, `controller/orders.rs::update_order_after_fill`, `controller/position.rs::decrease_open_bids_and_asks` with update flag and zero clamps, `User::decrement_open_orders`, the focused fill receipt builder slice (`OrderAction`, `OrderActionExplanation`, `OrderActionRecord`, `get_order_action_record`), post-receipt counter cleanup helper `decrement_open_orders_after_full_fill`, Drift-shaped `FillMode` with `is_liquidation` / `is_ioc`, and the `FillMode::get_limit_price` helper path. Latest verification: `cargo fmt -- --check` and full `cargo test` passed on 2026-05-08 with 53 tests after `FillMode::get_limit_price`. R4/R5 foundation debt remains intentionally parked for this small order-progress/event/cleanup/fill-mode slice only; return to it before any full trade demo or M1 close. Do not mark full Ring 7 complete yet; AMM route, maker route, fees, keeper rewards, emit inside real fill flow, and real fill orchestration are still pending.
...
- [ ] Build Slice 1 - IN PROGRESS: order-progress helpers, fill receipt shape, counter cleanup, FillMode basics, and tests. Completed so far: unfilled amount including reduce-only safety, partial/full fill order progress, open bid/ask decrease with Drift-style update flag and clamps, `User::decrement_open_orders`, focused `OrderActionRecord` receipt builder for current AMM-style fill slice, `decrement_open_orders_after_full_fill` with partial/full/underflow tests, `FillMode` enum plus `is_liquidation` / `is_ioc`, `Order.slot`, `Order::has_auction`, `math::auction::calculate_auction_price`, simplified `Order::get_limit_price`, and `FillMode::get_limit_price` with PlaceAndTake auction-percentage tests. Still pending: emitting the receipt inside a real Drift-shaped fill path and the R8-owned position-update boundary; do not fake any of these with mini-only flow helpers.
...
- [ ] Study Slice 2 - IN PROGRESS: route-selection sub-slice locked (`do_orders_cross` idea, maker direction chooses comparison, maker price is final matched price, makers vs AMM route ordering, `AMM(Some(price))`, `AMM(None)`, empty route list, and one Alice pipeline story). Still pending: fees, keeper reward, cleanup/cancel paths, exact `OrderActionRecord` fields, and source-code bridge through `determine_perp_fulfillment_methods`.
