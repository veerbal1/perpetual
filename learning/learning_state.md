...
- Status: IN PROGRESS - Ring 7 Build Slice 1 is partially shipped. Completed Drift-mirrored helper slices: `Order::get_base_asset_amount_unfilled`, `Order::update_open_bids_and_asks`, `controller/orders.rs::update_order_after_fill`, `controller/position.rs::decrease_open_bids_and_asks` with update flag and zero clamps, `User::decrement_open_orders`, the focused fill receipt builder slice (`OrderAction`, `OrderActionExplanation`, `OrderActionRecord`, `get_order_action_record`), post-receipt counter cleanup helper `decrement_open_orders_after_full_fill`, and Drift-shaped `FillMode` with `is_liquidation` / `is_ioc`. Latest verification: user-reported `cargo test` passed on 2026-05-06 after FillMode tests. R4/R5 foundation debt remains intentionally parked for this small order-progress/event/cleanup/fill-mode slice only; return to it before any full trade demo or M1 close. Do not mark full Ring 7 complete yet; AMM route, maker route, `FillMode::get_limit_price`, fees, keeper rewards, emit inside real fill flow, and any real fill orchestration are still pending.
...
- [ ] Study Slice 2 - AMM route, maker route, `FillMode::get_limit_price`, fees, keeper reward, cleanup/cancel paths, exact `OrderActionRecord` fields.
...
- Ring 7 next - study `FillMode::get_limit_price` in a fresh tiny chunk: first explain auction percentage on `PlaceAndTake(bool, u8)`, then only after it lands decide whether to build the method. Do not mix this with full fill orchestration; keep R8/R9/R13/R23 dependencies parked.
